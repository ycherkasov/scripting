...
def main():
    ...
    for dict in all_dictionaries:
        new_dictionary = set(dict.read())
        logger.info("Size of appended dictionary [%s] is %d words" % (dict.name(), len(new_dictionary)))
        merged_set = merged_set.union(new_dictionary)

    merged_list = sorted(merged_set)

    for dict in all_dictionaries:
        logger.info("Write new dictionary to %s" % dict.path())
        dict.write(merged_list)
